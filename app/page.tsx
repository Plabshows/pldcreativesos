...
export default function Home(){
  ...
  const content=active==='Facturas'?<FinanceWorkspace query={query}/>:active==='Tareas'?<TasksWorkspace query={query}/>:active==='Propuestas'?<ProposalsWorkspace query={query}/>:active==='Leads'?<CrmWorkspace mode="leads" query={query}/>:active==='Pipeline'?<CrmWorkspace mode="opportunities" query={query}/>:active==='Clientes'?<ClientBoard query={query} onBack={()=>setActive('Mi día')}/>:active==='Talento'?<TalentDirectory query={query} onAdd={addArtist} onPayments={()=>setActive('Pagos')} onBack={()=>setActive('Mi día')}/>:active==='Pagos'?<ArtistProfiles query={query} onBack={()=>setActive('Mi día')}/>:active==='Eventos'?<EventBoard query={query} onBack={()=>setActive('Mi día')}/>:active==='Inventario'?<InventoryWorkspace query={query}/>:active==='Shows'?<ShowsModule shows={shows} query={query} onBack={()=>setActive('Mi día')} onAdd={addShow}/>:active==='Equipo'?<TeamModule onBack={()=>setActive('Mi día')}/>:null;
