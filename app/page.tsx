...
import {ShowKitWorkspace} from '@/components/show-kit-workspace';
import {useEffect, useMemo, useState} from 'react';
import {CalendarDays, Check, ChevronDown, CircleDollarSign, FileText, LayoutDashboard, Menu, Package, Plus, Search, Settings, Sparkles, Target, UserRound, Users, Zap} from 'lucide-react';
...
type Section='Leads'|'Mi día'|'Clientes'|'Pipeline'|'Eventos'|'Talento'|'Shows'|'Equipo'|'Tareas'|'Propuestas'|'Pagos'|'Facturas'|'Inventario';
...
const nav:[Section, typeof LayoutDashboard][]=[['Mi día',LayoutDashboard],['Clientes',Users],['Leads',Target],['Pipeline',Target],['Eventos',CalendarDays],['Inventario',Package],['Talento',Sparkles],['Shows',Sparkles],['Equipo',UserRound],['Pagos',CircleDollarSign],['Tareas',Check],['Propuestas',FileText],['Facturas',CircleDollarSign]];
...
export default function Home(){
  ...
  useEffect(()=>{if(access!=='allowed')return;void fetch('/api/workspaces',{cache:'no-store'}).then(async r=>r.ok?responseJson(r):null).then(data=>setWorkspaces(data?.workspaces||[])).catch(()=>undefined);},[access]);
...
function TeamModule({onBack}:{onBack:()=>void}){
 ...
 return <div className="data-module">
  ...
  <section className="team-invite">
   <div>
    <b>Crear otro espacio de trabajo</b>
    <p>El nuevo espacio tendrá sus propios clientes, eventos, artistas, tareas y facturación. Tú entrarás como administrador.</p>
   </div>
   <button className="primary-button" disabled={creating} onClick={()=>void createWorkspace()}>{creating?'Creando…':'Crear espacio'}</button>
  </section>
 </div>
}
