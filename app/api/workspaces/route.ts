...
export async function GET(){const a=await session();if('error'in a)return a.error;const {data,error}=await a.supabase.from('organization_members').select('organization_id,role,organizations(id,name,slug)').eq('user_id',a.userId);if(error)return NextResponse.json({error:'No se pudieron cargar los espacios.'},{status:500});const selected=(await cookies()).get('plab_org')?.value;const workspaces=(data||[]).map(row=>({id:row.organization_id,role:row.role,name:(row.organizations as unknown as {name?:string}|null)?.name||'Espacio sin nombre'})).sort((x,y)=>x.name==='Performance Lab'?-1:y.name==='Performance Lab'?1:x.name.localeCompare(y.name));const activeId=workspaces.find(w=>w.id===selected)?.id||workspaces.find(w=>w.name==='Performance Lab')?.id||workspaces[0]?.id||null;return NextResponse.json({activeId,workspaces});}
...
export async function DELETE(){const response=NextResponse.json({ok:true});response.cookies.delete('plab_org');return response;}
