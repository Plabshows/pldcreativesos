...
const schema=z.discriminatedUnion('action',[
 ...
 z.object({action:z.literal('providerExpense'),event_id:z.string().uuid(),supplier_id:z.string().uuid().nullable().optional(),supplier_name:z.string().trim().max(200).optional(),expense_id:z.string().uuid().optional(),amount_cents:z.number().int().min(0).max(100000000).nullable().optional(),remove:z.boolean().optional()})
]);

// Cost recalculation fails closed when its source queries fail.
...
export async function GET(){
 const a=await requireOrganization();if('error'in a)return a.error;const org=a.membership.organization_id;
  const queries=[
   ...
   a.supabase.from('shows').select('id,name,category,description').eq('organization_id',org).eq('active',true).order('name').range(0,999),
...
export async function POST(req:Request){
 ...
 if(b.action==='artistFee'){
  ...
  const result=await a.supabase.rpc('set_event_provider_expense',{
   ...
   target_supplier_name:b.supplier_name||null,target_amount:b.amount_cents??null,remove_expense:Boolean(b.remove)
  });
  if(result.error)return NextResponse.json({error:result.error.code==='P0001'?result.error.message:'No se pudo guardar el gasto completo. Comprueba que la actualización de Supabase esté aplicada.'},{status:409});
  return NextResponse.json({ok:true});
  ...
  if (b.kind === 'talent' && b.remove) {
   ...
   const [payCheck, expenseCheck] = await Promise.all([
    ...
    a.supabase.from('expenses').select('id,legacy_payment_id').eq('organization_id', org).eq('event_id', b.event_id).eq('talent_id', b.target).neq('status', 'cancelled').range(0,999)
   ]);

   if (payCheck.error || expenseCheck.error) return NextResponse.json({error:'No se pudo comprobar el historial financiero del artista.'},{status:400});
   const expenseIds=(expenseCheck.data||[]).map(expense=>expense.id);
   const expensePayments=expenseIds.length?await a.supabase.from('expense_payments').select('id').eq('organization_id',org).in('expense_id',expenseIds).limit(1):{data:[],error:null};
   if (expensePayments.error) return NextResponse.json({error:'No se pudo comprobar el historial financiero del artista.'},{status:400});

   if ((payCheck.data || expensePayments.data?.length) && !b.forceRemove) {
    return NextResponse.json({
     ok: false,
