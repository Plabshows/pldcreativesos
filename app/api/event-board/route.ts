...
export async function GET(){
 ...
 const queries=[a.supabase.from('events').select('*').eq('organization_id',org).order('event_date',{ascending:false,nullsFirst:false}).range(0,999),a.supabase.from('clients').select('id,company_name,group_id').eq('organization_id',org).is('deleted_at',null).order('company_name').range(0,999),a.supabase.from('talent').select('id,real_name,city,email,phone,notes,skills,deleted_at,aliases,tax_id,iban,billing_supplier_id,billing_confidence,identity_sources,merged_into').eq('organization_id',org).order('real_name').range(0,999),a.supabase.from('shows').select('id,name').eq('organization_id',org).eq('active',true).order('name').range(0,999),a.supabase.from('event_talent').select('*').eq('organization_id',org).range(0,999),a.supabase.from('event_shows').select('*').eq('organization_id',org).range(0,999),a.supabase.from('client_groups').select('id,name').eq('organization_id',org).order('position'),a.supabase.from('event_places').select('kind,name').eq('organization_id',org).order('name').range(0,999)];
 ...
 return NextResponse.json({groups:r[6].data,places:r[7].data,events:r[0].data,clients:r[1].data,talent:r[2].data,shows:r[3].data,assignments:r[4].data,showLinks:r[5].data,canEdit:['admin','producer'].includes(a.membership.role)});
}
