...
-- Correct provider totals without rewriting any historical event.
-- Keep the same sources as set_artist_budget and syncEventExpensesTotal.
...
begin
  ...
  if target_expense is not null then
    ...
    if remove_expense then
      ...
      select case when count(*) filter (where amount is null) > 0 then null
    else coalesce(sum(amount), 0) end into total
  from (
    select agreed_cost_cents as amount from public.event_talent
    where organization_id=target_org and event_id=target_event
    union all
    select total_cents from public.expenses
