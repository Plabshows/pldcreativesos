...
export function clientMetrics(client:InsightClient,d:InsightsData,currency='EUR',today=invoiceToday()){
 ...
 const eventFinancials=jobs.map(e=>{const inv=invoices.filter(i=>i.event_id===e.id),revenue=currency==='EUR'?e.income_cents??(inv.length?inv.reduce((s,i)=>s+i.total_cents,0):null):inv.reduce((s,i)=>s+i.total_cents,0);const expenses=d.expenses.filter(x=>x.event_id===e.id&&x.status!=='cancelled'&&!x.covered_by_expense_id&&x.currency===currency);const cost=expenses.length?(expenses.some(x=>x.total_cents===null)?null:expenses.reduce((s,x)=>s+x.total_cents!,0)):currency==='EUR'&&e.expenses_cents!==null&&e.other_expenses_cents!==null?e.expenses_cents+e.other_expenses_cents:null;return {id:e.id,revenue,cost,profit:cost!==null&&revenue!==null?revenue-cost:null}});
 const eventsRevenue=eventFinancials.some(e=>e.revenue===null)?null:eventFinancials.reduce((s,e)=>s+e.revenue!,0);
 const invoiced=invoices.reduce((s,i)=>s+i.total_cents,0);
 const revenue=jobs.length>0?eventsRevenue:invoiced;
 const reconciliationDiff=eventsRevenue===null?null:eventsRevenue-invoiced;
...
export function buildClientInsights(d:InsightsData,currency='EUR'){
 ...
 return Object.fromEntries(d.clients.map(c=>{const m=metrics[c.id],knownRevenue=cohort.map(m=>m.revenue).filter((v):v is number=>v!==null),economic=m.revenue===null?0:percentile(m.revenue,knownRevenue),jobsScore=percentile(m.jobs,cohort.map(m=>m.jobs));const score=m.revenue===null||m.paymentScore===null||c.strategic_importance===null||c.ease_of_work===null||!m.jobs?null:Math.round(.25*economic+.20*jobsScore+.20*m.paymentScore+.20*c.strategic_importance*20+.15*c.ease_of_work*20);return [c.id,{...m,economic,jobsScore,score,level:score===null?'':score>=85?'A':score>=70?'B':score>=55?'C':score>=40?'D':'E'}]}));
}
