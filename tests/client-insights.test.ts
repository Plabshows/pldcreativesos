...
describe('client metrics',()=>{
 ...
 it('keeps currencies and unknown income separate',()=>{expect(clientMetrics(client,data,'USD').revenue).toBe(0);const unknown=clientMetrics(client,{...data,invoices:[],events:[{...event,income_cents:null}]});expect(unknown.profit).toBeNull();expect(unknown.revenue).toBeNull();expect(unknown.reconciliationDiff).toBeNull()});
