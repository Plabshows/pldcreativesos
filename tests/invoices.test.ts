...
describe('invoice balances',()=>{
 it('does not treat a legacy paid label as evidence of receipt',()=>{expect(invoiceBalance({...invoice,status:'paid'},[]).status).toBe('unverified');expect(invoiceBalance({...invoice,status:'paid'},[]).received).toBe(0);expect(invoiceBalance({...invoice,status:'paid'},[{...payment,amount_cents:10000}]).status).toBe('paid')});
