...
function calcEventCompleteness(e:Event,data:Data){
 const hasClient=Boolean(e.client_id),hasDate=Boolean(e.event_date),hasVenueOrCity=Boolean(e.venue||e.city),hasStatus=Boolean(e.status),hasShows=data.showLinks.some(a=>a.event_id===e.id)||Boolean(e.requested_entertainment),hasArtists=data.assignments.some(a=>a.event_id===e.id),hasPrice=(e.income_cents??0)>0,hasCosts=(e.expenses_cents??0)>0||(e.other_expenses_cents??0)>0,hasInvoice=e.billing_type==='cash'||Boolean(e.invoice_number),hasPaid=e.client_paid===true;
